  ...
  actions: {
    async getMyOrdersFromDB({commit},id) {
            try {
                const response = await fetch(`http://daw.deei.fct.ualg.pt/~a64514/EXAME/api/orders.php?customer_id=${id}`)
    ...
    async newOrder({commit}, newOrder) {         
          try {
              const response = await fetch('http://daw.deei.fct.ualg.pt/~a64514/EXAME/api/orders.php', {
                  method: 'POST',
