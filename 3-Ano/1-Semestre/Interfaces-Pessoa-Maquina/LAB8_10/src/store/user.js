    ...
    actions: {
        async userExists({commit}, user) {
			try {
				...
				const response = await fetch(`http://daw.deei.fct.ualg.pt/~a64514/LAB8_10/api/users.php?email=${user.email}`, {
					method: 'GET',
				})
        ...
        async addUser({state}) {
			try {
                ...
                const response = await fetch('http://daw.deei.fct.ualg.pt/~a64514/LAB8_10/api/users.php', {
					method: 'POST',
        ...
        async loginUser({commit}, user) {
			try {
				...
				const response = await fetch(`http://daw.deei.fct.ualg.pt/~a64514/LAB8_10/api/users.php?email=${user.email}&password=${user.password}`)
				const data = await response.json()
				if ( data == null) {
        ...
        async logoutUser({commit},session_id) {
			try {
				...
				const response = await fetch(`http://daw.deei.fct.ualg.pt/~a64514/LAB8_10/api/users.php?session_id=${session_id}`)
				const data = await response.json()
                console.log('received data:',data)
                commit('logoutUser')
				return true
