...
class MovementRig(Object3D):
    ...
    def update(self, input_object, delta_time):
        ...
        """if input_object.is_key_pressed(self.KEY_FORWARDS):
            self.translate(0, 0, -move_amount)
        if input_object.is_key_pressed(self.KEY_LEFT):
            self.translate(-move_amount, 0, 0)
        if input_object.is_key_pressed(self.KEY_RIGHT):
            self.translate(move_amount, 0, 0)
        if input_object.is_key_pressed(self.KEY_BACK):
            self.translate(0, 0, move_amount)"""
